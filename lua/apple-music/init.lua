--- apple-music/init.lua
--- Public API entry point for the apple-music.nvim plugin.
...
-- ---------------------------------------------------------------------------
-- Setup: called by lazy.nvim via opts / config.
-- opts = {
--   auto_start   = true,   -- start the background refresh timer automatically
--   notify_track = false,  -- show a vim.notify when the track changes
-- }
-- ---------------------------------------------------------------------------
M.config = {
  auto_start   = true,
...
function M.setup(opts)
  ...
  vim.api.nvim_create_user_command("AppleMusicInfo", function()
    ...
    if s.title == "" then
      ...
      vim.notify(
        string.format(
          "%s%s\n  %s  %s\n   %s",
          status_icon, s.title, s.artist, "·", s.album
        ),
        vim.log.levels.INFO,
        { title = " Apple Music" }
      )
  ...
  -- Optionally notify on track change.
...
-- ---------------------------------------------------------------------------
-- Re-export the most useful bits of the music module.
-- ---------------------------------------------------------------------------
